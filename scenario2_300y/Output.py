...
class Output:
	...
	print >>outfile,"%d update timesteps per year," %params.update,
	    ...
	    print >>outfile, "\n",
        if params.pmethod == 0:
            print >>outfile,"parameter variation method: 'between model runs'",
            print >>outfile,"(seeds:",
            for i in range(params.runs):
		print >>outfile,"r%d=%d" %(i+1,params.seed+i),
	    print >>outfile,")"
        if params.pmethod == 1:
            print >>outfile,"parameter variation method: 'between cells'",
	    print >>outfile,"(initial seed: %d)" %params.seed
        if params.pmethod == 2:
            print >>outfile,"parameter variation method: 'between timesteps'",
	    print >>outfile,"(initial seed: %d)" %params.seed
        if params.pmethod == 3:
            print >>outfile,\
		  "parameter variation method: 'between cells and timesteps'",
	    print >>outfile,"(no initial seed)"
...
#        ncfile.close()
    ...
    # Creating empty dictionary for cellavs for each variable.

    # cellavs is the average cell value across runs.
    ...
    def coralmapTXT(self,path,reefmap,coralmaps,sc_ben):
