...
class Map:
    ...
    def reefmap(self,pathname,forc_hurr,forc_cm,hmax_sr,cmmax_sr):
        ...
        if self.init_errors == 0:
            ...
            self.reefmap.reshape # self.reefmap is now a numpy array of the map

        if self.init_errors == 0:
            self.width = self.reefmap.shape[1] # get width and length (height?) of map
            self.length = self.reefmap.shape[0]
            self.srrange = self.reefmap.max() # get possible  subregions
            if self.srrange > 10:
                self.error_text.append(
            ...
            self.reeftotal = 0
            for ii in range(self.length): # looping through each pixel. length first
                for jj in range(self.width): # then width
                    for i in range(self.srrange+1)[1:]: # looping through possible subregions, skipping 0
                        if self.reefmap[ii,jj] == i: # if that pixel belongs to subregion[i]
                            self.subregs[i-1] +=1 # add 1 to that class (i-1 because 0 does not have representation in self.subregs)
                            self.subregid.append(i) # add subregion id to self.subregid
                            self.reeftotal +=1 # add 1 to self.reeftotal
            if self.reeftotal == 0:
                self.error_text.append(
