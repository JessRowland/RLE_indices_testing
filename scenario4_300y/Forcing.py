...
class Forcing:
    ...
    def hforcID(self,reefmap,max_sr,min_cat,max_cat,years,i):
        ...
        forcid = [0 for c in range(reefmap.reeftotal)]
        forclist = {}
        strength = np.random.choice(range(1,6),1,p=[0.4,0.5,0.04,0.04,0.02])
        if self.opt == 2:
            p = float(i)/float(years)
            r = random()
            if r < p and strength != max_cat:
                strength += 1
        if strength == 1: prop = 0.25
        elif strength == 2: prop = 0.25
        elif strength == 3: prop = 0.5
        elif strength == 4: prop = 0.75
        elif strength == 5: prop = 0.75
