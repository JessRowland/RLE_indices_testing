...
def Inputs(inputpath,outputpath):
   ...
   valid_names = ["scenario","region","res_MAR","res_SCS","years",
                  ...
                  "forc_fish","fmin_MAR","fmax_MAR","fmin_MAR2","fmax_MAR2",
                  "fmin_SCS","fmax_SCS","fmin_user","fmax_user","forc_df",
                  "forc_nut","forc_sed"]
         ...
         if(len(splits) == 2):
            ...
            if(varname in valid_names):
               if(len(splits) == 2):
                  ...
                  'fmin_user','fmax_user','fmin_MAR2','fmax_MAR2']):
   ...
   if optd['equilibration'] < 0 or type(optd['equilibration']) != int:
      ...
      print >>error_log,'(current value of "%s" is invalid)' \
            %optd['equilibration']
      init_errors += 1
   if optd['runs'] <= 0 or optd['runs'] > 1000 or type(optd['runs']) != int:
      if init_errors == 0:
            print >>error_log,'Errors in options file'
      print >>error_log,\
            '** Input for "runs" should be a positive integer less than 1000'
      print >>error_log,'(current value of "%s" is invalid)' %optd['runs']
      init_errors += 1
   ...
   if optd['forc_hurr'] in [1,2]:
      ...
      if optd['hmax_cat'] < optd['hmin_cat']:
         ...
         init_errors += 1
   if optd['forc_cm'] not in [0,1,2,3,4]:
      if init_errors == 0:
         print >>error_log,'Errors in options file'
      print >>error_log,\
            '** Input for "forc_cm" should be either "0", "1", "2", "3", or "4"'
   ...
   if optd['forc_cm'] == 1:
      if optd['cmfreq'] <= 0 or type(optd['cmfreq']) != int:
         ...
         init_errors += 1
   if optd['forc_cm'] == 4:
       if optd['cmfreq2'] <= 0 or type(optd['cmfreq2']) != int:
          if init_errors == 0:
             print >>error_log,'Errors in options file'
          print >>error_log,'** Input for "cmfreq2" should be a positive integer'
          print >>error_log,'(current value of "%s" is invalid)' %optd['cmfreq2']
          init_errors += 1
   if optd['forc_fish'] not in [0,1,2,3,4]:
      if init_errors == 0:
         print >>error_log,'Errors in options file'
      print >>error_log,\
            '** Input for "forc_fish" should be either "0", "1", "2","3" or "4"'
      print >>error_log,'(current value of "%s" is invalid)' %optd['forc_fish']
      init_errors += 1
   if optd['forc_fish'] in [1,4]:
      if optd['region'] == 0:
         if optd['fmin_MAR'] < 0:
            if init_errors == 0:
               print >>error_log,'Errors in options file'
         ...
         if optd['forc_fish'] == 4:
             if optd['fmin_MAR2'] < 0:
                if init_errors == 0:
                   print >>error_log,'Errors in options file'
                print >>error_log,\
                '** Input for "fmin_MAR2" should be greater than or equal to zero'
                print >>error_log,'(current value of "%s" is invalid)' \
                %optd['fmin_MAR2']
                init_errors += 1
             if optd['fmax_MAR2'] < 0:
                if init_errors == 0:
                   print >>error_log,'Errors in options file'
                print >>error_log,\
                '** Input for "fmax_MAR2" should be greater than or equal to zero'
                print >>error_log,'(current value of "%s" is invalid)' \
                %optd['fmax_MAR2']
                init_errors += 1
             if optd['fmax_MAR2'] < optd['fmin_MAR2']:
                if init_errors == 0:
                   print >>error_log,'Errors in options file'
                print >>error_log,\
                   '** Input for "fmax_MAR2" should greater than or equal to'
                print >>error_log,'the input for "fmin_MAR2"'
                init_errors += 1
      if optd['region'] == 1:
         if optd['fmin_SCS'] < 0:
            if init_errors == 0:
   ...
   foptions['f2'] = [fmin_MAR2,fmax_MAR2]
   if region == 0:
   ...
   # The above line loads in the map, reshapes into numpy ndarray, gives
   # attributes and checks that the parameters are sensible.
   ...
   # Pickle inputs
   p = open('_pickle/scenario','w'); pickle.dump(scenario,p); p.close()
   p = open('_pickle/filenames','w'); pickle.dump(filenames,p); p.close()
   p = open('_pickle/reefmap','w'); pickle.dump(reefmap,p); p.close()
   p = open('_pickle/params','w'); pickle.dump(params,p); p.close()
   p = open('_pickle/transitionCs','w'); pickle.dump(transitionCs,p); p.close()
   p = open('_pickle/transitionF','w'); pickle.dump(transitionF,p); p.close()
   p = open('_pickle/transitionU','w'); pickle.dump(transitionU,p); p.close()
   p = open('_pickle/transition','w'); pickle.dump(transition,p); p.close()
   p = open('_pickle/forcing_nut','w'); pickle.dump(forcing_nut,p); p.close()
   p = open('_pickle/forcing_sed','w'); pickle.dump(forcing_sed,p); p.close()
   p = open('_pickle/forcing_hurr','w'); pickle.dump(forcing_hurr,p); p.close()
   p = open('_pickle/forcing_cm','w'); pickle.dump(forcing_cm,p); p.close()
   p = open('_pickle/forcing_f','w'); pickle.dump(forcing_f,p); p.close()
   p = open('_pickle/forcing_df','w'); pickle.dump(forcing_df,p); p.close()
...
def Simulate():
   ...
   for run in range(params.runs):
      print 'run ' + str(run+1) + ' of ' + str(params.runs)
      ...
      if model.errors == 0:
          ...
          for var in variables:
              ...
              # This is where it takes only the map for the final year.
   ...
   p = open('_pickle/mcavs','w'); pickle.dump(mcavs,p); p.close()
   p = open('_pickle/mcCIs','w'); pickle.dump(mcCIs,p); p.close()
   p = open('_pickle/mcavsSR','w'); pickle.dump(mcavsSR,p); p.close()
   p = open('_pickle/mcCIsSR','w'); pickle.dump(mcCIsSR,p); p.close()
   p = open('_pickle/cellavs','w'); pickle.dump(cellavs,p); p.close()
