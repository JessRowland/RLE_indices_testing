...
class Simulation:
	...
	"""C  -- coral cover (proportion: sum of brooding and spawning corals)
	  ...
	  catchH,catchPs,catchPl
        ...
        # Looping through every cell that has coral
        for r in range(reefmap.reeftotal):
            # Looping through every subregion
    ...
    def model(self,reefmap,params,variables,transition,transitionCb,
	          ...
	          forcing_hurr,forcing_cm,forcing_f,forcing_df,run):
	...
	"""
	    ...
	    self.catchPl.append([])
            ...
            if forcing_cm.opt == 1:
                # Change here with new forcing_cm.opt == 4 where forcing
                # changes after 100 years
                if year > 1:
                    #seed(params.seed)
            ...
            elif forcing_cm.opt == 4:
                # Forcing changes after 100 years
                if 1 < year < 100 or 200 <= year < 300:
                    #seed(params.seed)
                    seed((params.seed + 0.001)*100 + run + 1)
                    for s in range(i):
                        rand = random()
	    ...
	    rho_Pl = [0 for r in range(reefmap.reeftotal)]
			...
			if year == 0:
			    ...
			    fishing = forcing_f.sched[year][reefmap.subregid[r]\
                    ...
                    elif forcing_f.opt == 1:
                        # pdb.set_trace()
                        fishing = self.p['f']*reefmap.cell_area
                    else:
                        if forcing_f.opt == 4: # Only possible option left
                            # pdb.set_trace()
    ...
    def averages(self,reefmap,params,variables,sc_ben,sc_con):
    ...
    def averagesSR(self,reefmap,params,variables,sc_ben,sc_con):
	...
	for var in variables:
	    self.avsSR[var] = (array(self.avsSR[var])).reshape(reefmap.srrange,
			       params.years)
